...
NS_LOG_COMPONENT_DEFINE ("Aodv1RoutingTable");
...
RoutingTableEntry::RoutingTableEntry (Ptr<NetDevice> dev, Ipv4Address dst, bool vSeqNo, uint32_t seqNo,
                                      Ipv4InterfaceAddress iface, uint16_t hops, Ipv4Address nextHop, Time lifetime, uint16_t priority)
...
uint16_t
RoutingTableEntry::GetPriority () const
{
  return m_priority ;
}
...
RoutingTable::RoutingTable (Time t)
  : m_badLinkLifetime (t), m_maxHops (1)
{
}
...
bool
RoutingTable::UpdateEntry (RoutingTableEntry & rt)
{
...
RoutingTable::Print (Ptr<OutputStreamWrapper> stream) const
{
  ...
  Purge (table);
  *stream->GetStream () << "\nAODV1 Routing table\n"
                        << "Destination\tGateway\t\tInterface\tFlag\tExpire\t\tHops\tPriority\n";
  for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i =
