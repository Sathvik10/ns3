 ...
 *
 * Based on
 *      NS-2 AODV1 model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      AODV1-UU implementation by Erik Nordström of Uppsala University
 *      http://core.it.uu.se/core/index.php/AODV1-UU
 *
...
#include "aodv1-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("Aodv1RoutingProtocol");

namespace aodv1 {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for AODV1 control traffic
const uint32_t RoutingProtocol::AODV1_PORT = 654;

/**
* \ingroup aodv1
* \brief Tag used by AODV1 implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::aodv1::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("Aodv1")
...
{
  static TypeId tid = TypeId ("ns3::aodv1::RoutingProtocol")
...
RoutingProtocol::Start ()
{
  ...
  Simulator::Schedule (MilliSeconds(10000),&RoutingProtocol::UpdateTable,
                                this);

}

void
RoutingProtocol::UpdateTable()
{
        std::map<Ipv4Address, RoutingTableEntry>* table = m_routingTable.GetTable();
        uint16_t max = 0;
        for (std::map<Ipv4Address, RoutingTableEntry>::iterator i =
         table->begin (); i != table->end (); ++i)
                { 
                        if (i->second.GetHop() > max)
                        {
                                max = i->second.GetHop();
                        }
                }
        m_routingTable.m_maxHops = max; 
        
        for (std::map<Ipv4Address, RoutingTableEntry>::iterator i =
         table->begin (); i != table->end (); ++i)
                {
                   
                        uint16_t NewPriority = (uint16_t)(((i->second.GetHop()*4)/max));
                       
                        if (i->second.GetPriority() != NewPriority)
                        {
                       // RoutingTableEntry newEntry (/*device=*/ i->second.GetOutputDevice(), /*dst=*/ i->second.GetDestination(), 
                         //               /*validSeqNo=*/ i->second.GetValidSeqNo (), /*seqno=*/ i->second.GetSeqNo (),
                           //             /*iface=*/i->second.GetInterface (),
                      //  /*hop=*/ i->second.GetHop(), /*nextHop=*/i->second.GetNextHop (), /*lifeTime=*/ i->second.GetLifeTime(),NewPriority);

                        // m_routingTable.UpdateEntry (newEntry);
                        
                        i->second.SetPriority(NewPriority);
                        
                        /*RoutingTableEntry rt;
                        if (m_routingTable.LookupValidRoute (i->second.GetDestination(), rt))
                        {
                                if (rt.GetPriority() == NewPriority)
                                        std::cout<< " worked \n";
                        }*/
                        
                        }
                }
        //Ptr<OutputStreamWrapper> rsm = Create<OutputStreamWrapper> (&std::cout);
     // m_routingTable.Print(rsm);
        Simulator::Schedule (MilliSeconds(5000),&RoutingProtocol::UpdateTable,this);

}

uint32_t
RoutingProtocol::PriorityOutput (const Ipv4Header &header)
{
...
uint8_t
RoutingProtocol::PriorityDestination (const Ipv4Address &dest)
{
  Ipv4Address dst = dest;
  RoutingTableEntry rt;
  uint32_t priority;
  if (m_routingTable.LookupValidRoute (dst, rt))
    {
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No aodv1 interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  // AODV1 is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == AODV1_PORT)
                    {
                      // AODV1 packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("AODV1 does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv1, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), AODV1_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv1, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), AODV1_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("AODV1 does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in AODV1 operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when AODV1 does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // AODV1 needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available AODV1 interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid AODV1 source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  // Send RREQ as subnet directed broadcast from each interface used by aodv1
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (AODV1TYPE_RREQ);
...
RoutingProtocol::SendTo (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
  ...
  uint8_t priority = PriorityDestination(destination);
  SocketPriorityTag priorityTag;
  priorityTag.SetPriority (priority);
  packet->ReplacePacketTag (priorityTag);
  
  socket->SendTo (packet, 0, InetSocketAddress (destination, AODV1_PORT));
...
void
RoutingProtocol::RecvAodv1 (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("AODV1 node " << this << " received a AODV1 packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("AODV1 message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (AODV1TYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), AODV1_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (AODV1TYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), AODV1_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (AODV1TYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, AODV1_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (AODV1TYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (AODV1TYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, AODV1_PORT));
        }
    }
}
